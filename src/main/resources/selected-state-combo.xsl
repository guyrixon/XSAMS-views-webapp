...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template match="xsams:AtomicState">
    <xsl:if test="@stateId=$stateID">
      <xsl:call-template name="atomic">
        <xsl:with-param name="state"   select="."/>
        <xsl:with-param name="ion"     select=".."/>
        <xsl:with-param name="isotope" select="../.."/>
  ...
  <xsl:template name="atomic">
    <xsl:param name="state"/>
    <xsl:param name="ion"/>
    <xsl:param name="isotope"/>
    <xsl:param name="atom"/>
    <h2>Species</h2>
    <p>
      <xsl:choose>
        <xsl:when test="$isotope/xsams:IsotopeParameters/xsams:MassNumber">
          <xsl:text>Isotope: </xsl:text>
          <sup><xsl:value-of select="$isotope/xsams:IsotopeParameters/xsams:MassNumber"/></sup>
    ...
    <p>
      ...
      <xsl:text> (</xsl:text>
      <xsl:value-of select="$isotope/xsams:InChIKey"/>
      <xsl:text>)</xsl:text>
    </p>
    <p>
    ...
    <p>
      ...
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:StateEnergy"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Ionization energy: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:IonizationEnergy"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Lande factor: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:LandeFactor"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Quantum defect: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:QuantumDefect"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Lifetime: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:Lifetime"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Polarizability: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:Polarizability"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Statistical weight: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:StatisticalWeight"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Hyperfine constant A: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:HyperfineConstantA"/>
      </xsl:call-template>
    </p>
    <p>
      <xsl:text>Hyperfine constant B: </xsl:text>
      <xsl:call-template name="quantity-with-unit">
        <xsl:with-param name="quantity" select="$state/xsams:AtomicNumericalData/xsams:HyperfineConstantB"/>
      </xsl:call-template>
    </p>
